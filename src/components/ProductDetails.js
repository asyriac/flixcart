...
const ProductDetails = () => {
  ...
  const isAddedToCart = () => {
    ...
    return (
      <button className="btn btn-primary" onClick={() => handleAddToCart(productDetails)} disabled={!productDetails.inStock}>
        Add to cart
      </button>
    );
  };
        ...
        <div className="flex flex-center pt-1">
          <div className="card ">
            <div className="card-img-container">
              ...
              {!isAddedToWishlist && (
                <button className="btn btn-primary btn-icon btn-icon-sm card-img-overlay" onClick={() => handleAddToWishlist(productDetails)}>
                  <i className="far fa-heart"></i>
                </button>
              )}
              {isAddedToWishlist && (
                <button className="btn btn-primary btn-icon btn-icon-sm card-img-overlay" onClick={() => handleRemoveFromWishlist(presentInWishlist)}>
